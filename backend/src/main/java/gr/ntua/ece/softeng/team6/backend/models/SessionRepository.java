...
public interface SessionRepository extends JpaRepository<Session, SessionId> {
        ...
        @Query(value = "select * from session where start_date >= ?3 and end_date <= ?4 and charger_station_id = ?1 and vehicle_license_plate = ?2 ORDER BY start_date, start_time", nativeQuery = true)
        public List<NameOnly> vehicleStation(Integer stationid,String vehicle, String startdate, String enddate);
        ...
        @Query(value = "select count(*) from session where charger_id = ?2 and charger_station_id = ?1", nativeQuery = true)
        public Integer stationPointSessionCount(Integer station, Integer count);
