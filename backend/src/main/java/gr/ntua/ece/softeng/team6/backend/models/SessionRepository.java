...
public interface SessionRepository extends JpaRepository<Session, SessionId> {
        ...
        @Query(value = "select * from session where start_date >= ?3 and end_date <= ?4 and charger_id = ?2 and charger_station_id = ?1", nativeQuery = true)
        public List<NameOnly> perPoint(Integer stationid, Integer pointid, String startdate, String enddate);

        @Query(value = "select * from session where start_date >= ?2 and end_date <= ?3 and charger_station_id = ?1", nativeQuery = true)
        public List<NameOnly> perStation(Integer stationid, String startdate, String enddate);

        @Query(value = "select * from session where start_date >= ?2 and end_date <= ?3 and vehicle_license_plate = ?1", nativeQuery = true)
